...
namespace json
{
    std::string JSONTokenType_to_string(JSONTokenType jtt)
    {
        ...
        // Adding return str to avoid warning
        return "";
    }
        ...
        for (int i = 0; i < raw_json.length(); i++)
        {
            ...
            for (auto lexer : generic_lexers)
            {
                if (auto [token, new_index, error] = lexer(raw_json, i); i != new_index)
                {
                    if (error.length())
                    {
                        std::vector<JSONToken> emptyVec;
                        auto t = std::make_tuple(emptyVec, error);
                        return t;
                    }
            ...
            std::vector<JSONToken> emptyVec;
            return {emptyVec, format_error("Unable to lex", raw_json, i)};
        }
    ...
    std::tuple<JSONValue, std::string> parse(std::string source)
    {
        ...
        if (error.size())
        {
            ...
            auto t = std::make_tuple(emptyValue, error);
            return t;
        }
